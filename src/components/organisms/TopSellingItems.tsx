              ...
              >
                ...
                <div className="overflow-y-auto p-6 space-y-6">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    {[
                      { label: "Category", value: selectedItem.category, icon: Trophy },
                      { label: "Sales", value: String(selectedItem.sales), icon: BarChart3 },
                      { label: "Revenue", value: formatMoney(selectedItem.revenue), icon: BadgePercent },
                      { label: "Trend", value: `${selectedItem.trend >= 0 ? "+" : ""}${selectedItem.trend.toFixed(1)}%`, icon: TrendingUp },
                    ].map((field) => (
                  ...
                  <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                    <div className="rounded-[24px] border border-gray-100 bg-white shadow-sm overflow-hidden">
                      <div className="border-b border-gray-100 px-5 py-4">
                        <h4 className="text-sm font-bold text-text-primary">Performance Summary</h4>
                        <p className="mt-1 text-xs text-text-secondary">A quick look at how this item is performing.</p>
                      </div>
                      <div className="space-y-3 p-5 text-sm">
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-text-secondary">Top rank</span>
                          <span className="font-medium text-text-primary">#{selectedRank || 1}</span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-text-secondary">Category</span>
                          <span className="font-medium text-text-primary">{selectedItem.category}</span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span className="text-text-secondary">Trend direction</span>
                          <span className={cn("font-medium", selectedItem.trend >= 0 ? "text-success-primary" : "text-error-primary")}>{selectedItem.trend >= 0 ? "Growing" : "Declining"}</span>
                        </div>
                      </div>
                    </div>

                    <div className="rounded-[24px] border border-gray-100 bg-brand-primary/5 p-5">
                      <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand-accent">Revenue Impact</p>
                      <p className="mt-3 text-3xl font-bold text-text-primary">{formatMoney(selectedItem.revenue)}</p>
                      <p className="mt-2 text-sm text-text-secondary">
                        This item is currently one of the strongest contributors in the selected sales period.
                      </p>
                    </div>
                  </div>
                </div>
