          ...
          {activeTab === "profile" ? (
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-bold text-text-secondary uppercase tracking-wider">Weekly Schedule</h3>
                <span className="text-xs font-medium text-brand-primary bg-orange-50 px-2 py-1 rounded-md">This Week</span>
              </div>
              
              <div className="space-y-2">
                {[
                  { day: "Monday", time: "9:00 AM - 5:00 PM", status: "shift" },
                  { day: "Tuesday", time: "9:00 AM - 5:00 PM", status: "shift" },
                  { day: "Wednesday", time: "Day Off", status: "off" },
                  { day: "Thursday", time: "11:00 AM - 7:00 PM", status: "shift" },
                  { day: "Friday", time: "11:00 AM - 7:00 PM", status: "shift" },
                  { day: "Saturday", time: "12:00 PM - 8:00 PM", status: "shift", current: true },
                  { day: "Sunday", time: "12:00 PM - 8:00 PM", status: "shift" },
                ].map((schedule, i) => (
                  <div 
                    key={i} 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-2 rounded-full ${
                        schedule.status === "off" ? "bg-gray-300" : 
                        schedule.current ? "bg-brand-primary" : "bg-green-500"
                      }`} />
                      <span className={`text-sm font-medium ${schedule.current ? "text-brand-primary" : "text-text-primary"}`}>
                        {schedule.day}
                      </span>
                    </div>
                    <span className={`text-sm ${
                      schedule.status === "off" ? "text-text-tertiary font-medium italic" : "text-text-secondary"
                    }`}>
                      {schedule.time}
                    </span>
                  </div>
