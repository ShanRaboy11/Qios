...
export const OrderReceiptModal = ({ onClose, orderId = "QIOS-9921" }: OrderReceiptModalProps) => {
  const { cart, cartTotal } = useCart();
  const receiptRef = useRef<HTMLDivElement>(null);

  // Helper to generate the jagged edge at the bottom
  const JaggedEdge = () => (
                ...
                <div className="b2 font-medium text-[#2D2D2D] leading-tight">
                  ...
                  {(item.selectedSize !== "s1" || item.selectedModifiers.length > 0) && (
                    <div className="text-[12px] text-black/50 mt-0.5 font-normal">
                      {[item.selectedSize !== "s1" ? "Large" : null, ...item.selectedModifiers.map(m => "Add-on")].filter(Boolean).join(", ")}
                    </div>
                  )}
                </div>
