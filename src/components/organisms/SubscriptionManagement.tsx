...
const FEATURE_DESCRIPTIONS: Record<string, string> = {
  "Browser-Based Ordering": "Access the digital menu through QR code scanning without requiring login.",
  "Text-Based AI Concierge": "A chat interface for customers to type orders and get recommendations.",
  "Menu Viewing & Item Customization": "Browse items, select predefined meals, and choose add-ons or sizes.",
  "Real-Time Price Calculation": "Automatically compute total cost based on selected items and options.",
  "Order Confirmation & QR Generation": "Review order summary and generate a QR code with order ID.",
  ...
  "QR Code Order Retrieval": "Scan customer-generated QR codes to retrieve order details.",
  "Order Modification & Validation": "Adjust items within an active order and verify authenticity.",
  "Payment Confirmation": "Confirm and finalize customer payments.",
  "Order Queue Management": "Send validated orders to the preparation dashboard and update status.",
  "Transaction Logging": "Record completed transactions into revenue records.",
  "Unit-Based Inventory Tracking": "Track current stock levels by fixed unit quantity per item.",
  "Measurement-Based Inventory Tracking": "Track stock in grams/mL based on recipe matrix.",
  "Automated Stock Deduction": "Automatically deduct stock upon order finalization.",
  "Physical Stock Input & Variance Reports": "Enter actual end-of-day counts and compute discrepancies.",
  "Shrinkage Alerts": "Flag unusual mismatches and missing stock levels.",
  "Live Revenue Dashboard": "View real-time total sales and daily income summaries.",
  "Sales Reports Generation": "Generate and export daily, weekly, or monthly transaction reports.",
  "Order Velocity Analytics": "Monitor average preparation time and service speed.",
  "Staff Activity Monitoring": "Track number of transactions processed by each cashier.",
  "Cancellation & Void Monitoring": "Detect excessive voids or suspicious transaction patterns.",
  "Admin Authentication": "Secure login for store owners or managers.",
  "Role & Permissions Management": "Create accounts, assign roles, and toggle feature access for staff.",
};

const INITIAL_PLANS: SubscriptionPlan[] = [
  {
    id: "p1", name: "Basic", color: "bg-[#ffc670]", badge: "Starter Ready",
    priceMonthly: "1,499", priceAnnually: "15,290",
    features: {
  ...
  {
    id: "p2", name: "Business", color: "bg-[#ff5269]", badge: "Most Popular",
    priceMonthly: "3,499", priceAnnually: "35,690",
    features: {
      ...
      employee_ops: { "Employee Authentication": true, "QR Code Order Retrieval": true, "Order Modification & Validation": true, "Payment Confirmation": true, "Order Queue Management": true, "Transaction Logging": true },
      inventory: { "Unit-Based Inventory Tracking": true, "Measurement-Based Inventory Tracking": false, "Automated Stock Deduction": true, "Physical Stock Input & Variance Reports": false, "Shrinkage Alerts": false },
      analytics: { "Live Revenue Dashboard": true, "Sales Reports Generation": true, "Order Velocity Analytics": false, "Staff Activity Monitoring": false, "Cancellation & Void Monitoring": false },
      admin_controls: { "Admin Authentication": true, "Role & Permissions Management": true },
    },
  },
  {
    id: "p3", name: "Enterprise", color: "bg-[#1fad66]", badge: "Premium Suite",
    priceMonthly: "7,999", priceAnnually: "81,590",
    features: {
      customer: { "Browser-Based Ordering": true, "Text-Based AI Concierge": true, "Menu Viewing & Item Customization": true, "Real-Time Price Calculation": true, "Order Confirmation & QR Generation": true, "Order Status Viewing": true },
      employee_ops: { "Employee Authentication": true, "QR Code Order Retrieval": true, "Order Modification & Validation": true, "Payment Confirmation": true, "Order Queue Management": true, "Transaction Logging": true },
...
const PRESET_COLORS = [
  "bg-[#ffc670]", "bg-[#ff5269]", "bg-[#1fad66]",
  "bg-[#3b82f6]", "bg-[#8b5cf6]", "bg-text-primary",
];
...
export default function SubscriptionManagement() {
  ...
  const handleDiscard = () => { if (activePlan) setDraftPlan(JSON.parse(JSON.stringify(activePlan))); };
  ...
  const handleDuplicate = () => {
    ...
    const newPlan = { ...JSON.parse(JSON.stringify(activePlan)), id: `p${Date.now()}`, name: `${activePlan.name} (Copy)` };
    setPlans([...plans, newPlan]);
    setSelectedPlanId(newPlan.id);
  };
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row gap-6 min-h-[700px]">
        ...
        <div className="w-full md:w-[280px] flex-shrink-0 flex flex-col gap-3">
          <Button variant="primary" className="w-full" leftIcon={<Plus size={18} />} onClick={() => setIsCreatePlanModalOpen(true)}>
            New Plan
          </Button>
            ...
            {filteredPlans.map((plan) => (
              <div
                ...
                className={cn(
                  ...
                  draggedPlanId === plan.id && "opacity-50 border-dashed border-2 border-brand-primary",
                )}
              >
                ...
                <div className="flex flex-col min-w-0">
                  <span className={cn("b2 font-bold transition-colors truncate", selectedPlanId === plan.id ? "text-text-primary" : "text-text-primary/80")}>
                    {plan.name}
                  </span>
                  {plan.badge && <span className="b5 text-text-secondary truncate mt-0.5">{plan.badge}</span>}
                </div>
              </div>
            ))}
          </div>
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex-1 w-full max-w-2xl flex gap-4 md:gap-6">
                    <div className="flex-1">
                      ...
                      <Input value={draftPlan.name} onChange={(e) => setDraftPlan({ ...draftPlan, name: e.target.value })} className="text-lg !bg-white/80 !py-1.5 !h-10" />
                    </div>
                    <div className="flex-1">
                      <label className="b4 font-bold text-text-secondary mb-2 block uppercase tracking-wider">Badge (Optional)</label>
                      <Input value={draftPlan.badge} onChange={(e) => setDraftPlan({ ...draftPlan, badge: e.target.value })} placeholder="e.g. Most Popular" className="text-sm !bg-white/80 !py-1.5 !h-10" />
                    </div>
                  </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                  <div className="flex-1 max-w-[200px]">
                    ...
                    <div className="relative">
                      ...
                      <Input value={draftPlan.priceMonthly} onChange={(e) => setDraftPlan({ ...draftPlan, priceMonthly: e.target.value })} className="pl-7 !bg-white/80 !py-1.5 !h-10" />
                    </div>
                  </div>
                  <div className="flex-1 max-w-[200px]">
                    ...
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 b2 font-bold text-text-secondary">₱</span>
                      <Input value={draftPlan.priceAnnually} onChange={(e) => setDraftPlan({ ...draftPlan, priceAnnually: e.target.value })} className="pl-7 !bg-white/80 !py-1.5 !h-10" />
                    </div>
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex gap-2 sm:gap-3 flex-nowrap pb-1">
                      {PRESET_COLORS.map((color) => (
                        <button key={color} onClick={() => setDraftPlan({ ...draftPlan, color })} className={cn("w-7 h-7 sm:w-8 sm:h-8 flex-shrink-0 rounded-full shadow-md transition-transform duration-200 hover:scale-110 flex items-center justify-center", color, draftPlan.color === color && "ring-4 ring-brand-primary/30 scale-110")}>
                ...
                <div className="flex flex-col gap-6 pb-4">
                  {showTemplateReminder && (
                    <div className="bg-warning-secondary/30 border border-warning-primary/30 rounded-xl md:rounded-[24px] p-4 flex items-start gap-3">
                      ...
                      <button onClick={() => setShowTemplateReminder(false)} className="ml-auto text-warning-primary/60 hover:text-warning-primary transition-colors"><X size={16} /></button>
                    </div>
                  )}

                  {Object.entries({
                    "Customer Features": { key: "customer" as keyof Features, data: draftPlan.features.customer, icon: <Smartphone size={18} /> },
                    "Employee Operations": { key: "employee_ops" as keyof Features, data: draftPlan.features.employee_ops, icon: <Users size={18} /> },
                    "Inventory Management": { key: "inventory" as keyof Features, data: draftPlan.features.inventory, icon: <Package size={18} /> },
                    "Analytics & Reporting": { key: "analytics" as keyof Features, data: draftPlan.features.analytics, icon: <LineChart size={18} /> },
                    "Admin Controls": { key: "admin_controls" as keyof Features, data: draftPlan.features.admin_controls, icon: <Shield size={18} /> },
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-text-primary/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl md:rounded-[32px] w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            <div className="px-6 md:px-8 py-4 md:py-6 flex items-center justify-between border-b border-black/[0.05] flex-shrink-0">
              ...
              <button onClick={() => setIsCreatePlanModalOpen(false)} className="text-text-secondary hover:text-text-primary transition-colors p-1"><X size={20} /></button>
            </div>
            <div className="p-6 md:p-8 overflow-y-auto">
              ...
              <div className="flex flex-col gap-3 mb-6">
                {INITIAL_PLANS.map((template) => (
                  <button key={template.id} onClick={() => handleConfirmCreatePlan(template)} className="flex items-center gap-4 p-4 rounded-xl border border-black/[0.05] hover:border-brand-primary/50 hover:bg-brand-primary/5 transition-all text-left group">
              ...
              <Button variant="outline" className="w-full" onClick={() => handleConfirmCreatePlan()}>Skip & Start from Scratch</Button>
            </div>
          </div>
        </div>
      )}
