...
export const SessionCard = ({
  ...
  return (
    <div className={cn(
      ...
      isActive ? "border border-brand-accent/20 bg-brand-primary/5" : "border border-gray-100 bg-white hover:bg-gray-50"
    )}>
      <div className="flex items-center gap-4">
        <div className={cn(isActive ? "text-brand-accent" : "text-text-secondary")}>
          {icon}
        </div>
        <div>
          <h4 className="font-bold text-text-primary">{device}</h4>
          <p className="text-xs text-text-secondary mt-0.5">{location} • {status}</p>
        </div>
      </div>
      {isActive ? (
