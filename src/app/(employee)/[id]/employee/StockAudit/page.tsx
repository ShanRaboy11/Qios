...
export default function StockAudit() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: allItemsList.length, audited: auditedCount, remaining: allItemsList.length - auditedCount };
  ...
  const filteredData = useMemo(() => {
    ...
    Object.entries(STOCK_DATA).forEach(([category, items]) => {
      const filtered = items.filter(item => item.name.toLowerCase().includes(query) || category.toLowerCase().includes(query));
    ...
    <div className="w-full min-h-screen pb-40 bg-bg-primary kds-fade-in">
      ...
      <div className="max-w-6xl mx-auto px-6 mt-6 space-y-12">
        {Object.entries(filteredData).map(([categoryName, items]: any, catIdx) => (
          <motion.section key={categoryName} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: catIdx * 0.1 }} className="space-y-6">
            <div className="flex items-center gap-4">
              <h3 className="b4 font-bold text-brand-accent uppercase tracking-widest">{categoryName}</h3>
                  ...
                  <div key={item.id} className={cn(
                    ...
                    <div className="flex-1 space-y-1">
                      <div className="flex items-center gap-2">
                        <h4 className={cn("b3 font-bold", isFinished ? "text-success-primary" : "text-text-primary")}>{item.name}</h4>
                        {isFinished && <CheckCircle2 size={16} className="text-success-primary" />}
                      </div>
                      <p className="b4 text-text-secondary flex items-center gap-2">
                        <span className="px-2 py-0.5 bg-bg-primary rounded-md font-semibold text-[10px] uppercase border border-kds-border-warm">{item.category}</span>
                    ...
                    <div className="lg:w-32 text-center mt-4 lg:mt-0">
                      <p className="lg:hidden b5 font-bold text-text-secondary uppercase mb-1">System</p>
                      <p className="b1 text-text-primary tabular-nums">{item.systemCount}</p>
                    </div>

                    <div className="lg:w-64 flex flex-col items-center mt-4 lg:mt-0">
                      <p className="lg:hidden b5 font-bold text-text-secondary uppercase mb-1">Actual Found</p>
                      <div className="flex items-center gap-2">
                        <button onClick={() => adjustCount(item.id, physicalCounts[item.id], -1)} className="w-8 h-8 rounded-full bg-bg-primary border border-kds-border-warm flex items-center justify-center hover:bg-brand-primary transition-colors">
                          <Minus size={14} />
                        </button>
                        <div className="relative">
                          <input type="text" value={physicalCounts[item.id] || ""} onChange={(e) => handleInputChange(item.id, e.target.value)} placeholder="0.00" className="w-20 h-10 text-center b1 font-bold rounded-xl border-2 border-kds-border-warm focus:border-brand-accent focus:outline-none transition-all" />
                          {!isFinished && (
                              <button 
                    ...
                    <div className="lg:w-40 flex flex-col items-center mt-4 lg:mt-0">
                      <span className="lg:hidden b5 font-bold text-text-secondary uppercase mb-1">Variance</span>
                      <Badge 
                          color={(variance.status === "match" || variance.status === "increase") ? "success" : variance.status === "pending" ? "primary" : "error"}
                    ...
                    <div className="lg:w-20 flex justify-end mt-4 lg:mt-0">
                       <Button variant={hasNote ? "primary" : "ghost"} size="icon" shape="rounded" onClick={() => setActiveNoteId(item.id)} className="relative text-text-primary">
                        <FileText size={20} />
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-white/90 backdrop-blur-md border-t border-kds-border-warm z-40">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <Button variant="outline" shape="pill" onClick={() => setPhysicalCounts({})} leftIcon={<RotateCcw size={16}/>} className="b2 font-semibold">Reset</Button>
            <Button variant="accent" shape="pill" className="flex-1 sm:flex-none px-12 py-6 shadow-coral b2 font-bold" disabled={!isAuditComplete}>Complete Audit</Button>
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {activeNoteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/20">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative bg-white rounded-[32px] w-full max-w-lg shadow-2xl p-8 border border-kds-border-warm">
              ...
              <textarea autoFocus value={notes[activeNoteId] || ""} onChange={(e) => setNotes(prev => ({ ...prev, [activeNoteId!]: e.target.value }))} placeholder="Reason for discrepancy..." className="w-full h-40 p-5 bg-bg-primary border-2 border-kds-border-warm rounded-2xl focus:border-brand-accent focus:outline-none b2 resize-none transition-all" />
              <div className="mt-8">
                <Button onClick={() => setActiveNoteId(null)} className="w-full py-4 shadow-gold b2 font-bold">Save Note</Button>
              </div>
            </motion.div>
          </div>
        )}
