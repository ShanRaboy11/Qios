    ...
    >
      <div className="flex justify-between items-start mb-4">
        <div className="space-y-1">
          <p className="text-text-secondary b4 font-bold uppercase tracking-widest opacity-80">
            {title}
          </p>
  ...
  return (
    <div className="min-h-screen bg-bg-primary p-6 md:p-12 space-y-10 font-inter kds-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
        <div className="space-y-1">
          <h1 className="h2 text-text-primary tracking-tighter">
            Executive Overview
          </h1>
          <p className="b2 text-text-secondary">
            Real-time performance tracking and commercial revenue analysis.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="flex gap-4">
            {["Growth Trends", "Peak Periods", "Sales Fluctuations"].map(
              (tab) => (
                <Button
                  key={tab}
                  ...
                  className={cn(
                    "px-6 font-normal",
                    activeTab === tab ? "shadow-lg scale-105" : "bg-white/50",
                  )}
                >
                  {tab}
                </Button>
          ...
          >
            <div className="flex justify-between items-center mb-12">
              <h2 className="h4 text-text-primary tracking-tight">
                {activeTab}
              </h2>
            ...
            <div className="relative h-[220px] md:h-[280px] mb-2">
              <div className="absolute left-0 h-full flex flex-col justify-between b4 font-medium text-gray-400 z-0 pointer-events-none">
                {["$8.8m", "$8.6m", "$8.4m", "$8.2m", "$8m"].map((v) => (
                  <div key={v} className="h-0 flex items-center">
                    <span>{v}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="absolute right-4 h-full flex flex-col justify-between b4 font-medium text-gray-400 z-0 pointer-events-none">
                {["150", "120", "60", "30", "0"].map((v) => (
                  <div
                    key={v}
                    className="h-0 flex items-center justify-end w-full"
                  >
                    <span>{v}</span>
                  </div>
                ))}
                  ...
                  >
                    <AnimatePresence>
                      {hoveredMonth === item.month && (
                        <motion.div
                          ...
                          exit={{ opacity: 0 }}
                          className="absolute -top-14 bg-zinc-900 text-white px-2 py-1.5 md:px-3 md:py-2 rounded-lg text-[9px] md:text-[10px] z-50 shadow-xl pointer-events-none whitespace-nowrap border border-white/10"
                        >
                          <div className="space-y-1">
                    ...
                    <div
                      className={cn(
                        ...
                        hoveredMonth && hoveredMonth !== item.month
                    ...
                    >
                      <motion.div
                        ...
                        className="w-3.5 sm:w-6 md:w-8 bg-[#FFD77A] rounded-t-md shadow-sm group-hover/item:brightness-105"
                      />
                      <motion.div
                        initial={{ height: 0 }}
                        animate={{ height: `${item.cost}%` }}
                        className="w-4 sm:w-6 md:w-8 bg-[#FF5269] rounded-t-md shadow-sm group-hover/item:brightness-105"
                      />
                    </div>
                  </div>
                ))}
            ...
            <div className="flex flex-wrap justify-center gap-4 sm:gap-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] md:b5 font-bold text-text-secondary uppercase">
                  Lifetime Value
                </span>
              </div>
              <div className="flex items-center gap-2">
        ...
        >
          <div className="p-8 border-b border-gray-50 flex justify-between items-center">
            <h4 className="h5 font-medium text-text-primary">
              Recent Activity
            </h4>
            <div className="flex items-center gap-2">
              <Button
              ...
              {RECENT_ACTIVITY.map((activity, index) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="b2 font-bold text-text-primary group-hover:text-brand-accent transition-colors">
                          {activity.id}
                        </span>
                        <Badge variant="subtle" color={activity.color}>
                          {activity.type}
                        </Badge>
                      </div>
                      <p className="b5 text-text-secondary font-bold uppercase tracking-tighter">
                        {activity.entity}
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="b5 text-text-tertiary font-medium">
                      {activity.time}
                    </p>
                  </div>
                </motion.div>
