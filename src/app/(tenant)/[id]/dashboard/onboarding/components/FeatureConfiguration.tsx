...
const CustomLockedToggle = ({ 
  ...
  const knobColor = variant === "rose" ? "#FF5269" : "#ffc670";
  const trackColor = variant === "rose" ? "bg-[#FF5269]/20" : "bg-[#ffc670]/20";

  return (
    <div className={cn(
      // FIXED: Changed h-full to h-[30px] to prevent stretching
      "w-[50px] h-[30px] rounded-full transition-colors duration-300 flex items-center px-1 shrink-0",
      isOn ? trackColor : "bg-neutral-100"
    )}>
      <div className={cn(
        ...
        <svg width="22" height="22" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <g clipPath="url(#clip0_1778_5359)">
            <path d="M6.28516 11.7859C6.28516 10.6747 6.28516 10.1191 6.63035 9.77391C6.97555 9.42871 7.53113 9.42871 8.6423 9.42871H11.7852C12.8963 9.42871 13.4519 9.42871 13.7971 9.77391C14.1423 10.1191 14.1423 10.6747 14.1423 11.7859C14.1423 12.897 14.1423 13.4526 13.7971 13.7978C13.4519 14.143 12.8963 14.143 11.7852 14.143H8.6423C7.53113 14.143 6.97555 14.143 6.63035 13.7978C6.28516 13.4526 6.28516 12.897 6.28516 11.7859Z" stroke="#FFF9EF" strokeWidth="0.785714"/>
            <path d="M7.85742 9.42899V8.64328C7.85742 7.34146 8.91275 6.28613 10.2146 6.28613C11.0003 6.28613 11.786 6.67899 12.1789 7.4647" stroke="#FFF9EF" strokeWidth="0.785714" strokeLinecap="round"/>
            <path d="M10.2148 11V12.5714" stroke="#FFF9EF" strokeWidth="0.785714" strokeLinecap="round"/>
          </g>
          <defs>
...
export function FeatureConfig({ onFinish }: FeatureConfigProps) {
  ...
  return (
    <div className="flex flex-col gap-10 animate-in fade-in slide-in-from-right-8 duration-500 w-full max-w-[520px] pb-10">
      ...
      {/* SECTION 1: INVENTORY MODE */}
      <div className="space-y-4">
        <h3 className="text-[24px] font-medium font-figtree text-[var(--color-text-primary)]">
            ...
            <div 
              ...
              className={cn(
                ...
                inventoryMode === item.id 
                  ? "bg-white border-[var(--color-brand-primary)] shadow-[0_0_0_4px_rgba(255,198,112,0.05)]" 
                  : "bg-[#F8F8F8] border-transparent hover:bg-[#F0F0F0]"
              )}
            >
              <CustomLockedToggle isOn={inventoryMode === item.id} variant="rose" />
              <div className="flex flex-col">
                <span className="text-[17px] font-bold text-[var(--color-text-primary)] font-figtree leading-tight">
                  {item.label}
                </span>
                <span className="text-[13px] text-[var(--color-text-secondary)] font-inter mt-0.5">
                  {item.desc}
                </span>
              </div>
            </div>
