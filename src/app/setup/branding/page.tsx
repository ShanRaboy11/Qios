...
export default function BrandingSetupPage() {
  ...
  const [suggestedThemes, setSuggestedThemes] = useState<Array<{primary: string, secondary: string, accent: string}>>([]);
    ...
    if (imageRef.current) {
      try {
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          const hex = '#' + [fR, fG, fB].map(x => x.toString(16).padStart(2, '0')).join('');
        ...
        if (sortedColors.length > 0) {
          ...
          if (c1 && c2 && c3) {
            ...
            themes.push({ primary: c1, secondary: chroma(c1).set('hsl.l', 0.95).hex(), accent: c2 });
            themes.push({ primary: c2, secondary: chroma(c2).set('hsl.l', 0.95).hex(), accent: c3 });
            themes.push({ primary: c3, secondary: chroma(c1).set('hsl.l', 0.95).hex(), accent: c1 });
          } else if (c1 && c2) {
            // Logo has 2 colors: generate theme variants based on these two
            themes.push({ primary: c1, secondary: chroma(c1).set('hsl.l', 0.95).hex(), accent: c2 });
            themes.push({ primary: c2, secondary: chroma(c2).set('hsl.l', 0.95).hex(), accent: c1 });
            themes.push({ 
              primary: c1, 
            ...
            themes.push({ 
              primary: c1, 
              secondary: chroma(c1).set('hsl.h', '+30').set('hsl.l', 0.95).hex(), 
              accent: chroma(c1).set('hsl.h', '-30').saturate(2).hex() 
            });
            themes.push({ 
              primary: c1, 
              secondary: chroma(c1).set('hsl.h', '+180').set('hsl.l', 0.95).hex(), 
  ...
  const fonts = [
    ...
    { id: "playfair", name: "Playfair Display", css: "font-serif", desc: "Elegant & Classic" },
    { id: "roboto", name: "Roboto", css: "font-mono", desc: "Technical & Crisp" },
  ];
  ...
  const handleFinish = async () => {
    ...
    setTimeout(() => {
      setIsSaving(false);
      router.push("/"); // Direct them to home/dashboard
    }, 1500);
  };
        ...
        <div className="p-6 md:p-10 flex-1">
          <div className="mb-10">
            ...
            <p className="text-text-secondary">
              Let's customize how your menus and kiosks will look to your customers.
            </p>
          </div>
          ...
          <div className="space-y-10 pb-10">
            ...
            <div className="space-y-6">
              <SectionHeader title="Brand Theme" className="mb-0 py-2 border-gray-100" />
                    ...
                    {suggestedThemes.map((t, index) => (
                      <button
                        ...
                        className={cn(
                          ...
                          theme.primary === t.primary && theme.secondary === t.secondary && theme.accent === t.accent
                            ? "border-text-primary bg-gray-50 scale-[1.02]"
                            : "border-gray-100 hover:border-gray-200"
                        )}
                      >
                        <div className="flex-1 h-10 rounded-lg shadow-sm" style={{ backgroundColor: t.primary }} title={`Primary: ${t.primary}`} />
                        <div className="flex-1 h-10 rounded-lg shadow-sm" style={{ backgroundColor: t.secondary }} title={`Secondary: ${t.secondary}`} />
                        <div className="flex-1 h-10 rounded-lg shadow-sm" style={{ backgroundColor: t.accent }} title={`Accent: ${t.accent}`} />
                      </button>
                    ))}
                  </div>
                  <p className="text-xs text-text-secondary mt-3">
                    Click a theme above to apply the extracted colors from your logo.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              <SectionHeader title="Typography" className="mb-0 py-2 border-gray-100" />
                ...
                {fonts.map((font) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2">
                      <Type size={18} className={fontFamily === font.id ? "text-text-primary" : "text-text-secondary"} />
                      <span className="font-semibold text-text-primary text-lg">
                        {font.name}
                      </span>
                    </div>
                    <p className="text-sm text-text-secondary">{font.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Layout Preferences */}
            <div className="space-y-4">
              <SectionHeader title="Menu Layout" className="mb-0 py-2 border-gray-100" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                {layouts.map((layout) => {
                  ...
                  return (
                    ...
                    >
                      <div className={cn(
                        ...
                        menuLayout === layout.id ? "text-white" : "bg-gray-100 text-text-secondary"
                      )} style={menuLayout === layout.id ? { backgroundColor: theme.primary } : {}}>
                        <Icon size={24} />
                      </div>
                      <div>
            ...
            {/* Logos & Media */}
            <div className="space-y-4">
              <SectionHeader title="Logos & Media" className="mb-0 py-2 border-gray-100" />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-2">
                <div className="space-y-3">
                  ...
                  >
                    ...
                    {logoUrl ? (
                      ...
                      <>
                        <Upload size={24} className="text-gray-400 mb-3 group-hover:text-text-primary transition-colors" />
                        <span className="text-sm font-medium text-text-primary">
                          Upload Logo
                        </span>
                        <span className="text-xs text-text-secondary mt-1">PNG, JPG</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl p-6 flex flex-col items-center justify-center bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer group h-40">
                    <Globe size={24} className="text-gray-400 mb-3" />
                    <span className="text-sm font-medium text-text-primary">
                      Upload Icon
                    </span>
                    <span className="text-xs text-text-secondary mt-1">32x32</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="flex-1 bg-gray-100 hidden md:flex flex-col items-center justify-center p-8 lg:p-12 relative overflow-hidden">
        ...
        <div className="w-full max-w-[380px] relative z-10">
          <div className="flex items-center justify-center gap-2 mb-8">
            ...
            <h3 className="text-sm font-bold tracking-widest text-gray-500 uppercase">Live Preview</h3>
          </div>
                ...
                {[1, 2, 3, 4, 5, 6].map((item) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      menuLayout === "grid" ? "flex-col" : "flex-row gap-4 items-center"
                    )}
                  >
                    <div className={cn(
                      "bg-gray-100 rounded-xl",
                      menuLayout === "grid" ? "w-full aspect-square mb-3" : "w-24 h-24 shrink-0"
                    )} />
                    <div className="flex-1 flex flex-col h-full">
                      <h5 className="font-semibold text-gray-900 text-sm mb-1">Signature Dish {item}</h5>
                      {menuLayout === "list" && (
                        <p className="text-xs text-gray-500 mb-2 line-clamp-2">A delicious description of this amazing dish that your customers will love.</p>
                      )}
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-white p-5 border-t border-gray-100 shadow-[0_-20px_40px_-15px_rgba(0,0,0,0.05)] z-20">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="bg-white w-7 h-7 rounded-full flex items-center justify-center text-sm" style={{ color: theme.primary }}>2</div>
